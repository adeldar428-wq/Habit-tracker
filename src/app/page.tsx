...
export default function Home() {
  ...
  const handleAdd = () => { setEditingHabit(null); setShowModal(true); };
  const handleEdit = (habit: Habit) => { setEditingHabit(habit); setShowModal(true); };
  const handleClose = () => { setShowModal(false); setEditingHabit(null); };
      ...
      {habits.length === 0 ? (
        ...
        <div className="px-4 mt-4">
          {showFilter && (
            <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 mb-4 -mx-1 px-1">
              ...
              {usedCategories.map(catId => {
                ...
                if (!cat) return null;
                return (
                  <CategoryPill key={catId} label={cat.label} emoji={cat.emoji}
                    active={activeCategory === catId} onClick={() => setActiveCategory(catId)} />
                );
              })}
            </div>
          )}
          ...
          {filteredHabits.length === 0 && activeCategory !== 'all' && (
            <div className="text-center py-12 text-white/30 text-sm">No habits in this category.</div>
          )}
        </div>
      )}

      {showModal && <HabitModal habit={editingHabit} onClose={handleClose} />}
    </main>
  );
}
